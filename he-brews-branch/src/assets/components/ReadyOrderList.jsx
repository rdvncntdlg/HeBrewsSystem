...
const ReadyOrderList = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch('https://hebrewscafeserver.onrender.com/api/ready-orders', {
        method: 'GET',
        headers: {
  ...
  const handleAcceptOrder = async (orderId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`https://hebrewscafeserver.onrender.com/api/orders/${orderId}/complete`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div>
    {orders.length === 0 ? (
      <div className="text-center">No ready orders</div>
    ) : (
      orders.map((order) => (
        <OrderCard
          key={order.order_id}
