...
class DetailAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final provider =
        FavoriteProvider.of(context); // Access the FavoriteProvider
    return Padding(
            ...
            onPressed: () {
              ...
              if (token.isNotEmpty) {
                provider.toggleFavorite(
                    product, token); // Toggle favorite with the token
              } else {
                print(
                    'Token is null or empty'); // Handle the case where token is invalid
              }
            },
            icon: Icon(
